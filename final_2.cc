...
#include <iostream>
#include <cmath>
#include <chrono>  // Include chrono library to measure real execution time
...
{
  // Start measuring overall real execution time for all scenarios
  ...
  {
    // Record the start time for the current scenario
    ...
    // Print node positions for debugging purposes
    ...
    // Set up high-traffic applications
    uint16_t port = 5000;

    // Each node sends data to multiple destinations
    ...
    // Start the applications at specified simulation times
    sinkApps.Start (Seconds (1.0));
    sourceApps.Start (Seconds (2.0));
    sinkApps.Stop (Seconds (30.0));
    sourceApps.Stop (Seconds (29.0));

    // Set up FlowMonitor to collect simulation statistics
    FlowMonitorHelper flowmon;
    Ptr<FlowMonitor> monitor = flowmon.InstallAll ();

    // Run the simulation with a stop time of 35 seconds
    ...
    for (auto iter = stats.begin (); iter != stats.end (); ++iter)
    {
      ...
      {
        double lossRatio = (iter->second.txPackets - iter->second.rxPackets) /
                           static_cast<double>(iter->second.txPackets);
    ...
    std::cout << "Real execution time for scenario with " << nNodes
              << " nodes: " << scenarioDuration.count() << " seconds" << std::endl;
