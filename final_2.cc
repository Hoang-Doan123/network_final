...
int main (int argc, char *argv[])
{
  ...
  for (uint32_t nNodes = 2; nNodes <= 30; nNodes += 2)
  {
    ...
    // Set up sources - each node sends to all other nodes for maximum contention
    for (uint32_t i = 0; i < nNodes; i++)
    {
      for (uint32_t j = 0; j < nNodes; j++)
      {
        ...
        OnOffHelper source ("ns3::UdpSocketFactory", InetSocketAddress (interfaces.GetAddress (j), port));
